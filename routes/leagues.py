    ...
    try:
        ...
        if 'founded_date' in data and data['founded_date']:
            try:
                ...
                return format_error(
                    "Invalid date format for 'founded_date'. Use ISO format (YYYY-MM-DDTHH:MM:SS)"
                ), 400

        # Create new league using SQLAlchemy model
        new_league = LeagueModel(name=data['name'],
...
def toggle_league_status(league_id):
    ...
    try:
        ...
        return format_response({
            "message":
            f"League status toggled to {'enabled' if league.enabled else 'disabled'}",
...
def get_popular_leagues():
    ...
    try:
        ...
        popular_leagues = LeagueModel.query.filter_by(enabled=True).order_by(
            desc(LeagueModel.popularity)).limit(limit).all()
